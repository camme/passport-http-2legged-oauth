// This file was reused from https://github.com/jaredhanson/passport-http-oauth
  ...
  if (comps) {
    for (var i = 0, len = comps.length; i < len; i++) {
      ...
      if (name.indexOf('oauth_') == 0) {
        params[name] = val;
      } else if (name.indexOf('xoauth_') == 0) {
        params[name] = val;
      }
    }
  }
 ...
 * @api private
 */
exports.normalizeParams = function(header, query, body, arraysWithParentesisOrNot) {
    if (typeof body == "boolean") {
        arraysWithParentesisOrNot = body;
        delete body;
    }
    arraysWithParentesisOrNot = arraysWithParentesisOrNot ? arraysWithParentesisOrNot : false;
    delete arguments[3];
    var mh = new MultiHash();
    for (var i = 0, len = arguments.length; i < len; i++) {
        var source = arguments[i];
        if (!source || typeof source == "boolean") { continue; }
        ...
        Object.keys(source).forEach(function(key) {
            ...
            if (typeof value == 'object') {
                for(var key2 in value){
                    if (arraysWithParentesisOrNot) {
                        mh.put(exports.encode(key + "[" + key2 + "]"), exports.encode(value[key2]));
                    }
                    else {
                        mh.put(exports.encode(key), exports.encode(value[key2])); 
                    }
                }
            }
            else {
                ...
                //console.log(value);
                // This is added to compensate if the siganture is encoded without false strings (omitted because of conversion)
                //value = value == 'false' ? '' : value;
                //value = value == '0' ? '' : value;
                mh.put(exports.encode(key), exports.encode(value));
